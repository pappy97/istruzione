...
export class UpdateCorsoComponent {
  ...
  getnotProf() {
    return this.doc
      .getAllDocenti()
      .filter((e) => !this.toUpdate.docenti.includes(e.id));
  }
  getProf() {
    return this.doc
      .getAllDocenti()
  ...
  updateCorso() {
    this.toUpdate.docenti = this.toUpdate.docenti.filter(
      (e) => !e.includes(this.remprof)
    );
